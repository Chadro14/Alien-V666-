...
senddocu(buffer)
    ...
    case 'autostatusview':{
            ...
            case 'bcgroup': {
                ...
                replygcxeon(`*Broadcast envoye avec succès dans les ${anu.length} Groupes*`)
            }
            break
            case 'getcase':
                if (!isCreator) return replygcxeon(mess.owner)
            ...
            case 'del': {
                ...
                if (!isBaileys) return replygcxeon('*Ce message n a pas ete envoye par un bot !*')
                xeonBotInc.sendMessage(m.chat, {
                    delete: {
                        remoteJid: m.chat,
                        fromMe: true,
            ...
            case 'linkgc':
                ...
                XeonBotInc.sendText(m.chat, `👽 *GROUP LINK INFO*\n👽 *Name :* ${groupMetadata.subject}\n☣️ *Group Owner :* ${groupMetadata.owner !== undefined ? '@' + groupMetadata.owner.split`@`[0] : 'Not known'}\n☣️ *ID :* ${groupMetadata.id}\n☣️— *Chat Link :* https://chat.whatsapp.com/${response}\n☣️ *Member :* ${groupMetadata.participants.length}\n`, m, {
                    detectLink: true
                })
                break
            case 'revoke':
...
break
            case 'afk':
                ...
                break
case 'play':  case 'song': {
if (!text) return replygcxeon(`Example : ${prefix + command} Trailer Son Alien-V666`)
...
case "xandroid": {
  ...
  replygcxeon("*VEUILLEZ PATIENTER, " + command + " LA MORSURE DU SOLEIL ☀️🔥 EST EN COURS DE PROCESSUS...*");
...
case "xios": {
  ...
  replygcxeon("*VEUILLEZ PATIENTER, " + command + " L'hyper virus👽 EST EN COURS DE PROCESSUS...*");
  await sleep(2000); // Adjusted sleep time for clarity
...
case "xios2":
  {
    ...
    replygcxeon("*VEUILLEZ PATIENTER, " + command + " Bankai Zanka No Tachi 🗡️ EST EN COURS DE PROCESSUS...*");
  ...
  replygcxeon("*VEUILLEZ PATIENTER, " + command + " Bankai -SenbonZakura Kageyoshi ⚔️ EST EN COURS DE PROCESSUS...*");
  await sleep(2000); // Adjusted sleep time for clarity
